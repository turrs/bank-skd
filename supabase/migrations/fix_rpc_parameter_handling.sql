-- Fix RPC parameter handling issue
-- The problem is that Supabase RPC calls need explicit parameter handling

-- Drop the existing function
DROP FUNCTION IF EXISTS public.validate_and_apply_voucher(VARCHAR(50), UUID, UUID);

-- Create the function with explicit parameter handling for RPC calls
...
BEGIN
    ...
    ELSE
        ...
        RAISE NOTICE 'Function now handles RPC parameters explicitly';
    END IF;
END $$;
